...
const TRANSFORM_CODE: Record<string, string> = {
    ...
    if (!rawData?.series || rawData.series.length < 2)
        return { series1Data: [], series2Data: [] };
    const housingStarts = rawData.series.find((s) => s.id === 'HOUSTNSA');
    const caseShiller = rawData.series.find((s) => s.id === 'CSUSHPISA');
    if (!housingStarts || !caseShiller)
        return { series1Data: [], series2Data: [] };
    return {
        series1Data: housingStarts.observations.map((o) => ({ date: o.date, value: parseFloat(o.value) })),
        series2Data: caseShiller.observations.map((o) => ({ date: o.date, value: parseFloat(o.value) })),
...
// Import all time series components
import TimeseriesBasic from '@/viz/components/recharts/generic/timeseries-basic-v1';
import TimeseriesDualAxis from '@/viz/components/recharts/generic/timeseries-dual-axis-v1';
import TimeseriesIndex from '@/viz/components/recharts/generic/timeseries-index-v1';
...
const variants: VariantConfig[] = [
    {
        id: 'basic',
        name: 'Basic',
        description: 'Simple time series line chart with recession period overlays and time range buttons.',
        sampleData: 'housing.json',
        componentPath: 'recharts/generic/timeseries-basic-v1',
        Component: TimeseriesBasic,
    },
    {
        id: 'dual-axis',
        name: 'Dual Axis',
        description: 'Time series with two Y-axes for comparing metrics at different scales.',
        sampleData: 'housing.json',
        componentPath: 'recharts/generic/timeseries-dual-axis-v1',
        Component: TimeseriesDualAxis,
    },
    {
        id: 'index',
        name: 'Index Comparison',
        description: 'Normalized index comparison where all series are rebased to 100.',
        sampleData: 'housing.json',
        componentPath: 'recharts/generic/timeseries-index-v1',
        Component: TimeseriesIndex,
    },
    {
        id: 'multiline',
        name: 'Multi-Line',
        description: 'Multi-series line chart for comparing trends across categories (Observable Plot).',
        sampleData: 'zillowmultiline.json',
        componentPath: 'plot/timeseries/multiline-v1',
        Component: MultiLine,
        isPlot: true,
    },
];
    ...
    return (
        <div className="relative">
            ...
            <div className="py-6 px-4 max-w-4xl mx-auto lg:mr-56 space-y-12">
                {/* Header */}
                <div className="space-y-4">
                    <h1 className="text-4xl font-black tracking-tight text-foreground">Time Series Charts</h1>
                    <p className="text-lg text-muted-foreground leading-relaxed max-w-2xl">
                        Line charts for temporal data with various configurations including recession overlays, dual axes, indexed comparisons, and multi-series comparisons.
                    </p>
